...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
    ...
    while True:
        ...
        if len(local_buffer):
            ...
            #send off the data to our request handler
...
def hexdump(src, length=16):
    ...
    for i in xrange(0, len(src), length):
        ...
        text = b''.join([x if 0x020 <= ord(x) < 0x7F else b'.' for x in s])
...
def receive_from(connection):
    ...
    try:
        # keep reading into the buffer until theres no more data or we timeout
        while True:
